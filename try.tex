...
\title{偏波の情報を加味した圧縮センシングによる埋設されたパイプの推定}
\author{廣瀬夏秋研究室\\
学籍番号03-210499 高原陽太}
...
になる。この図は$y$軸方向にx=0.1,地中から8cmのところに埋設したものである。また、地中12cmでの断面図が最も大きな反応が見られたため、
これについて比較する。
...
\chapter{総括と今後の展望}
\section{総括}
　本研究では圧縮センシングの際に偏波の情報を加味することで目標となるパイプの向きが推定でき、より計測点数を減らすことを
目的としているが、今回の実験により照射した偏波の向きがパイプの向きと平行または垂直であるかによって受信波の振幅と
位相は異なることが確かめられた。それゆえに両者を比較することでパイプの向きを推定することは可能であると期待できる。
\\　しかし、地表面による反射が非常に大きく、地中に電磁波が届きにくく、また受信データへのノイズとなってしまっているので
